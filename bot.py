...
class EmbedModal(discord.ui.Modal, title="Create Embed"):
    ...
    description_input = discord.ui.TextInput(
        label="Description",
        placeholder="Write your embed content here...",
        style=discord.TextStyle.paragraph,
        required=True,
        max_length=4000
    )

    def __init__(self, channel, color, image, thumbnail, user):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        parsed_color = color_map.get(self.color.lower().strip(), None)
        ...
        embed.set_footer(text=f"Sent by {self.user}", icon_url=self.user.display_avatar.url)
        ...
        await interaction.followup.send(f"Embed sent to {self.channel.mention}!", ephemeral=True)
...
async def embed_command(
    ...
    modal = EmbedModal(channel, color, image, thumbnail, interaction.user)
